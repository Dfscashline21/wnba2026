...
class SupabaseConnection:
    ...
    def __init__(self):
        ...
        self.port = os.getenv("SUPABASE_PORT") or "5432"
    ...
    def get_engine(self):
        ...
        if self._engine is None:
            logger.info(f"DB connect — host={self.host!r} user={self.user!r} dbname={self.dbname!r} "
                        f"password={'set' if self.password else 'NOT SET'}")
            try:
                # SUPABASE_HOST may be a full connection URL rather than a bare hostname
                if self.host and (self.host.startswith('postgresql://') or self.host.startswith('postgres://')):
                    logger.info("SUPABASE_HOST is a full connection URL — using it directly")
                    self._engine = create_engine(
                        self.host,
                        pool_pre_ping=True, pool_recycle=300,
                        pool_size=5, max_overflow=10, pool_timeout=60
                    )
                ...
                # Bare hostname — build URL from components
                if self.password:
                    logger.info("Attempting pooler connection (port 6543)...")
                    try:
                        pooler_url = URL.create("postgresql", username=self.user, password=self.password,
                                                host=self.host, port=6543, database=self.dbname)
                        self._engine = create_engine(pooler_url,
                        ...
                        with self._engine.connect() as conn:
                            version = conn.execute(text("SELECT version();")).fetchone()
                            logger.info(f"Pooler connection successful: {version[0]}")
                            return self._engine
                    except Exception as pooler_error:
                        logger.warning(f"Pooler failed: {pooler_error} — falling back to port 5432")
                        self._engine = None

                password = self.password or self.service_key
                logger.info("Attempting direct connection (port 5432)...")
                direct_url = URL.create("postgresql", username=self.user, password=password,
                                        host=self.host, port=5432, database=self.dbname)
                self._engine = create_engine(direct_url,
                ...
                with self._engine.connect() as conn:
                    version = conn.execute(text("SELECT version();")).fetchone()
                    logger.info(f"Direct connection successful: {version[0]}")
...
# Lazy singleton — not instantiated until first call
_supabase_conn: Optional[SupabaseConnection] = None

def _get_conn() -> SupabaseConnection:
...
def get_supabase_engine():
    ...
    return _get_conn().get_engine()

def get_db_connection():
    logger.warning("⚠️ get_db_connection() is deprecated.")
    return None
