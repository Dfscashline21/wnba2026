...
-- Analysis: Trend-Based Prediction Accuracy Evaluation
-- This analysis evaluates the accuracy of trend-based prop predictions across various dimensions
...
WITH accuracy_metrics AS (
    SELECT 
        ...
        CASE 
            WHEN overs_5 = 5 OR overs_5 = 0 THEN 'Perfect Trend'
            WHEN overs_5 = 4 OR overs_5 = 1 THEN 'Strong Trend'
            ELSE 'Mixed Trend'
        END as trend_strength_5,
        
        CASE 
            WHEN overs_10 = 10 OR overs_10 = 0 THEN 'Perfect Trend'
            WHEN overs_10 >= 9 OR overs_10 <= 1 THEN 'Strong Trend'
            ELSE 'Mixed Trend'
        END as trend_strength_10
        
    FROM {{ ref('trend_recommendations_with_actuals') }}
    GROUP BY 
        stat_type, confidence_level, recommendation_type, player_name, 
        team_abbreviation, season_year, odds_type, week_start, month_start, 
...
overall_summary AS (
    SELECT 
        'OVERALL' as category,
        'All Predictions' as subcategory,
        COUNT(*) as total_predictions,
...
recommendation_summary AS (
    SELECT 
        ...
        recommendation_type as subcategory,
        COUNT(*) as total_predictions,
        SUM(correct_predictions) as correct_predictions,
        ROUND(AVG(overall_accuracy_pct), 2) as accuracy_pct,
        recommendation_type as detail_value
    FROM accuracy_metrics
...
odds_type_summary AS (
    SELECT 
        'ODDS_TYPE' as category,
        odds_type as subcategory,
        COUNT(*) as total_predictions,
        SUM(correct_predictions) as correct_predictions,
        ROUND(AVG(overall_accuracy_pct), 2) as accuracy_pct,
        odds_type as detail_value
    FROM accuracy_metrics
    GROUP BY odds_type
),
...
all_analyses AS (
    ...
    UNION ALL
    SELECT * FROM recommendation_summary
    UNION ALL
...
ORDER BY 
    CASE 
        ...
        WHEN category = 'RECOMMENDATION_TYPE' THEN 2
        WHEN category = 'CONFIDENCE' THEN 3
        WHEN category = 'STAT_TYPE' THEN 4
        WHEN category = 'TREND_STRENGTH_5' THEN 5
        WHEN category = 'TREND_STRENGTH_10' THEN 6
        WHEN category = 'ODDS_TYPE' THEN 7
        WHEN category = 'TOP_PLAYERS' THEN 8
        WHEN category = 'TOP_TEAMS' THEN 9
        WHEN category = 'LINE_VALUE' THEN 10
        WHEN category = 'WEEKLY_TRENDS' THEN 11
        WHEN category = 'SEASON' THEN 12
        ELSE 13
    END,
    accuracy_pct DESC