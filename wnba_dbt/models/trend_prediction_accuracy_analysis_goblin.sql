...
-- Analysis: Trend-Based Prediction Accuracy Evaluation for GOBLIN Odds
-- This analysis evaluates the accuracy of trend-based prop predictions for goblin odds
...
overall_summary AS (
    SELECT 
        'OVERALL' as category,
        'All Goblin Predictions' as subcategory,
        COUNT(*) as total_predictions,
...
top_players AS (
    ...
    HAVING COUNT(*) >= 3  -- Minimum 3 predictions for goblin
...
ORDER BY 
    CASE 
        ...
        WHEN category = 'TOP_PLAYERS' THEN 7
        WHEN category = 'TOP_TEAMS' THEN 8
        WHEN category = 'LINE_VALUE' THEN 9
        WHEN category = 'WEEKLY_TRENDS' THEN 10
        WHEN category = 'SEASON' THEN 11
        ELSE 12
    END,
    accuracy_pct DESC