...
with overs as (select *
            ...
            ,"Assists", sum("Assists Over")/5 ::float as astover,"Blks+Stls",sum("Blks+Stls Over")/5 ::float as stocksover,"Fantasy Score",sum("Fantasy Score Over")/5 ::float as fantasyscoreover,"Points",sum("Points Over")/5 ::float as pointsover,"Pts+Asts",sum("Pts+Asts Over")/5 ::float as points_ast_over,"Pts+Rebs",sum("Pts+Rebs Over")/5 ::float as points_rebover,"Pts+Rebs+Asts",sum("Pts+Rebs+Asts Over")/5 ::float as praover
            ,"Rebounds",sum("Rebounds Over")/5 ::float as rebover,"Rebs+Asts",sum("Rebs+Asts Over")/5 ::float as reb_astover,"Turnovers",sum("Turnovers Over")/5 ::float as tovover
            from overs
