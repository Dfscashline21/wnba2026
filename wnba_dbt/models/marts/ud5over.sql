with overs as (select *
            ...
            ,"three_points_made",sum("3-PT Over") /5 ::float as threesover
            ,"assists", sum("Assists Over")/5 ::float as astover,"fantasy_points",sum("Fantasy Score Over")/5 ::float as fantasyscoreover
            ,"free_throws_made",sum("Free Throws Made Over")/5 ::float as ftmadeover
            ,"points",sum("Points Over")/5 ::float as pointsover
            ,"pts_asts",sum("Pts+Asts Over")/5 ::float as points_ast_over,"pts_rebs",sum("Pts+Rebs Over")/5 ::float as points_rebover
            ,"pts_rebs_asts",sum("Pts+Rebs+Asts Over")/5 ::float as praover
            ,"rebounds",sum("Rebounds Over")/5 ::float as rebover
            ,"rebs_asts",sum("Rebs+Asts Over")/5 ::float as reb_astover
