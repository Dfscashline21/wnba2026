...
-- Analysis: Trend-Based Prediction Accuracy Evaluation for STANDARD Odds - UNDER Only
-- This analysis evaluates the accuracy of trend-based prop predictions for standard odds UNDER recommendations
...
WITH accuracy_metrics AS (
    SELECT 
        ...
        CASE 
            WHEN overs_5 = 0 THEN 'Perfect Trend (0/5)'
            WHEN overs_5 = 1 THEN 'Strong Trend (1/5)'
            ELSE 'Mixed Trend (>1/5)'
        END as trend_strength_5,
        
        CASE 
            WHEN overs_10 = 0 THEN 'Perfect Trend (0/10)'
            WHEN overs_10 <= 1 THEN 'Strong Trend (0-1/10)'
            ELSE 'Mixed Trend (>1/10)'
        END as trend_strength_10
        
    FROM {{ ref('trend_recommendations_with_actuals') }}
    WHERE odds_type LIKE 'standard%'
    AND recommendation_type = 'HIGH_CONFIDENCE_UNDER'
    GROUP BY 
...
overall_summary AS (
    SELECT 
        'OVERALL' as category,
        'All Standard UNDER Predictions' as subcategory,
        COUNT(*) as total_predictions,
...
top_players AS (
    ...
    HAVING COUNT(*) >= 3  -- Minimum 3 predictions for standard UNDER
