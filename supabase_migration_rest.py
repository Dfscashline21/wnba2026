...
Handles data migration and validation (Database only)
...
class SupabaseMigration:
    """Handles migration from PostgreSQL to Supabase using REST API"""
    ...
    def migrate_database_tables(self):
        ...
        try:
            ...
            if legacy_engine is None:
                raise Exception("Could not connect to legacy PostgreSQL database")
    ...
    def validate_migration(self):
        ...
        for table in tables_to_validate:
            try:
                # Try to get a small sample of data to validate the table exists and has data
                df = self.supabase_rest.get_table_data(table, schema='wnba', limit=5)
                if df is not None and not df.empty:
                    ...
                    logger.error(f"❌ Table {table} validation failed - no data found")
