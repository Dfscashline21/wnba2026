...
def pull_mgm():
    ...
    today = dt.today()
    start_date = (today - timedelta(days=1)).strftime('%Y-%m-%dT08:00:00.000Z')
    end_date = (today + timedelta(days=1)).strftime('%Y-%m-%dT07:59:59.999Z')

    for propid in player_props_ids:
        try:
            url = f'https://api.oddsshopper.com/api/offers/{propid}/outcomes/live?state=NV&seasonStartYear=2024&startDate={start_date}&endDate={end_date}&selectedSportsbooks=&sortBy=Time&oddsExplore=false&edgeSportsbooks=Circa%2CFanDuel%2CPinnacle'
