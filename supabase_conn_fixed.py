...
Fixed version with proper connection pooling
        ...
        if self._engine is None:
            try:
                ...
                if self.url.startswith('db.'):
                    # Already using connection pooler URL, use pooler port
                    connection_string = f"postgresql://{self.user}:{self.service_key}@{self.url}:6543/{self.dbname}"
                else:
                    # Convert to connection pooler URL
                    project_id = self.host.replace('.supabase.co', '')
                    pooler_url = f"db.{project_id}.supabase.co"
                    connection_string = f"postgresql://{self.user}:{self.service_key}@{pooler_url}:6543/{self.dbname}"
