...
def get_adjusted_rates(players_today):
    ...
    baseline.to_csv(p / 'players.csv', index=False)
    
    baseline_rates = baseline[['TeamId','Name','TeamAbbreviation','Minutes','Usage','Assists','Rebounds']]
    team_totals = baseline_rates.groupby(['TeamAbbreviation']).sum(numeric_only=True).reset_index()
    ...
    try:
            ...
            adjusted.to_csv(p / 'adj_players.csv', index=False)
            adjusted_rates = adjusted[['TeamId','Name','TeamAbbreviation','Minutes','Usage','Assists','Rebounds','Points']]
            team_totals = adjusted_rates.groupby(['TeamAbbreviation']).sum(numeric_only=True).reset_index()
            ...
            rate_boosts.to_csv(p / 'boosts.csv', index=False)
    except:
