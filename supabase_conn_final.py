...
class SupabaseConnection:
    ...
    def __init__(self):
        ...
        # Create client with correct Supabase URL
        supabase_url = f"https://{self.host}" if self.host else self.url
        if supabase_url.startswith('db.'):
            # Convert pooler URL to regular Supabase URL
            project_id = supabase_url.replace('db.', '').replace('.supabase.co', '')
...
def get_supabase_client() -> Client:
    """Get Supabase client (replaces get_db_connection)"""
...
def get_db_connection():
    ...
    logger.warning("⚠️ get_db_connection() is deprecated. Use get_supabase_client() instead.")
