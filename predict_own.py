...
def predict_ownership():
        ...
        main.to_csv(str(p) + '\\wnba historical.csv')
        ...
        file_name = str(p) +  f" {todaysdate} WNBA Article.txt"
        
        with open(file_name, 'w') as file:
           file.write(article + valuearticle)
        
           
        file_name2 =str(p) +  f"{todaysdate} WNBA DK Article.txt"
        ...
        projupload.to_csv(str(p) +'\\wnbaload.csv')
